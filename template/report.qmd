...
FATTURA(
  ...
  Note,
  CF FK→PERSONA.CF,
  CodiceIntervento FK→INTERVENTO.CodiceIntervento,
  CodiceSessione FK→SESSIONE.CodiceSessione
)
...
MOTO(
  ...
  Note,
  CodiceModello FK→MODELLO.CodiceModello,
  CF FK→PERSONA.CF
)
...
RICAMBIO(
  ...
  Descrizione[0–1],
  PrezzoFornitore,
  CodiceFornitore FK→FORNITORE.CodiceFornitore
)
...
STOCCAGGIO(
  CodiceMagazzino FK→MAGAZZINO.CodiceMagazzino,
  CodiceRicambio FK→RICAMBIO.CodiceRicambio,
  PRIMARY KEY(CodiceMagazzino, CodiceRicambio)
)

APPARTENENZA(
  CodiceRicambio FK→RICAMBIO.CodiceRicambio,
  CodiceModello FK→MODELLO.CodiceModello,
  PRIMARY KEY(CodiceRicambio, CodiceModello)
)
...
UTILIZZO(
  ...
  Quantità,
  Prezzo,
  PRIMARY KEY(CodiceRicambio, CodiceIntervento)
)
