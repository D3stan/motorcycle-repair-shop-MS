...
class FinancialController extends Controller
{
    ...
    public function reports(Request $request): Response
    {
        ...
        // Revenue summary
        $totalRevenue = Invoice::whereBetween('issue_date', [$dateFrom, $dateTo])
            ->where('status', 'paid')
            ->sum('total_amount');
            
        $totalInvoices = Invoice::whereBetween('issue_date', [$dateFrom, $dateTo])->count();
        $paidInvoices = Invoice::whereBetween('issue_date', [$dateFrom, $dateTo])
            ->where('status', 'paid')
            ->count();
            
        $averageInvoiceAmount = $paidInvoices > 0 ? $totalRevenue / $paidInvoices : 0;
        ...
        while ($currentDate->lte($dateTo)) {
            $monthRevenue = Invoice::whereMonth('issue_date', $currentDate->month)
                ->whereYear('issue_date', $currentDate->year)
                ->where('status', 'paid')
                ->sum('total_amount');
                
            $monthInvoices = Invoice::whereMonth('issue_date', $currentDate->month)
                ->whereYear('issue_date', $currentDate->year)
                ->count();
        ...
        // Top revenue sources
        $topCustomersRevenue = User::where('type', 'customer')
            ->withSum(['invoices' => function($query) use ($dateFrom, $dateTo) {
            ...
            ->map(function ($customer) {
                return [
                    ...
                    'revenue' => (float) ($customer->invoices_sum_total_amount ?? 0),
                ];
            });
