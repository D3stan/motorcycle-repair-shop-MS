...
.code {sql}
    ...
    SELECT 
        (SELECT COUNT(*) FROM APPUNTAMENTI WHERE DataAppuntamento >= CURDATE()) as AppuntamentiOggi,
        (SELECT COUNT(*) FROM INTERVENTI WHERE DataFine IS NULL) as InterventiAperti,
        (SELECT COUNT(*) FROM SESSIONI WHERE Stato = 'in_corso') as SessioniInCorso,
        (SELECT SUM(Importo) FROM FATTURE WHERE Data >= DATE_SUB(NOW(), INTERVAL 30 DAY)) as RicaviMensili;
