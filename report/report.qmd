...
Operazione che permette la consultazione di una fattura esistente. Utilizzata per l'analisi delle ridondanze, confronta l'accesso diretto all'importo memorizzato con il ricalcolo dinamico dai ricambi utilizzati.
...
1. **Memorizzazione dell'importo totale nella fattura**: l'attributo `Importo` nella tabella `FATTURE` potrebbe essere considerato ridondante poiché calcolabile dinamicamente sommando i costi dei ricambi utilizzati (`UTILIZZI`) e della manodopera associata all'intervento.
...
Senza l'attributo `Importo`, ogni visualizzazione richiederebbe il ricalcolo del totale, accedendo a tutte le entità correlate per sommare i costi dei ricambi e della manodopera (qui non quantificata ma che richiederebbe ulteriori accessi).
...
**Conclusione**: Mantenere l'importo ridondante riduce significativamente il costo delle operazioni di lettura, giustificando la scelta progettuale a fronte di un leggero aumento del costo in fase di scrittura (calcolo effettuato una sola volta durante l'operazione OP 7).
